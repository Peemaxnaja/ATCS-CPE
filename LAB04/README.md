...
└── RAG-Project/
    ...
    │   └── golden_set.json            ชุดวัดผล 120 ข้อ x 4 รูปแบบคำถาม
    │
...
`build_golden_set.py` สุ่ม 120 ข้อจาก 364 chunks แบบกระจายหมวดละ 10 ข้อ แล้วแปลงคำถามต้นฉบับเป็น 4 รูปแบบ เพื่อดูว่าระบบทนคำถามที่ไม่ตรงตัวได้แค่ไหน
...
ทดสอบทุกข้อทุกรูปแบบ รวม 386 query ผลเต็มอยู่ใน [`outputs/eval_retrieval.json`](RAG-Project/outputs/eval_retrieval.json)

**ภาพรวม**

| วิธีค้น | Hit@1 | Hit@10 | MRR | nDCG@3 | ms/query |
| :--- | :---: | :---: | :---: | :---: | :---: |
| dense_only | 0.8368 | 0.9637 | 0.8803 | 0.8811 | 12.4 |
| **bm25_only** | **0.9663** | **1.0000** | **0.9821** | **0.9856** | **2.0** |
| hybrid (RRF) | 0.9119 | 1.0000 | 0.9426 | 0.9410 | 11.6 |

**แยกตามรูปแบบคำถาม (MRR)**

| วิธีค้น | verbatim | slang | partial | natural |
| :--- | :---: | :---: | :---: | :---: |
| dense_only | 0.9748 | 0.6934 | 0.7235 | 0.9831 |
| bm25_only | 1.0000 | 0.9808 | 0.9465 | 1.0000 |
| hybrid (RRF) | 0.9944 | 0.8465 | 0.8585 | 0.9958 |
...
แปลว่าสามในสี่รูปแบบวัด "การจับคำตรงตัว" มากกว่าวัดความเข้าใจความหมาย งานนี้จึงเข้าทาง BM25 ตั้งแต่ต้น ถ้าจะวัดของจริงต้องมีคำถามที่ **เขียนใหม่ด้วยคำคนละชุด** ซึ่งชุดนี้ยังไม่มี

รูปแบบที่ใกล้เคียงการวัดจริงที่สุดคือ `slang` เพราะเปลี่ยนคำจริง ๆ ไม่ใช่แค่ตัดหรือเติม — และเป็นรูปแบบที่ dense ทำได้แย่ที่สุด
...
### ตาราง SLANG_MAP ใช้ได้ผล แต่ชุดวัดผลไม่ได้เรียกมัน

จุดที่ต้องบอกให้ชัด — `eval_retrieval.py` เรียก `retriever.retrieve()` ตรง ๆ ไม่ผ่าน `RAGPipeline` ฟังก์ชัน `normalize_query()` ที่ทำหน้าที่แทนคำพูดด้วยศัพท์ทางทหารจึงไม่เคยถูกเรียกเลย ตัวเลข `slang` ข้างบนคือความสามารถของ retriever ล้วน ไม่ใช่ของระบบทั้งระบบ

ผมเลยวัดเพิ่มเองว่าถ้าใส่ `normalize_query()` เข้าไปก่อนค้นจะต่างแค่ไหน
...
ตารางแทนคำพูด 11 บรรทัดที่ไม่ใช้ AI เลย กู้คะแนน dense กลับมาได้เกือบทั้งหมด นี่เป็นผลที่ชัดที่สุดในใบงานนี้ และเป็นเหตุผลว่าทำไม query normalization ถึงคุ้มกว่าการไล่เปลี่ยนโมเดล embedding เป็นอย่างแรก

ในทางกลับกันมันก็แปลว่าตัวเลข `slang` ในตารางหลักประเมินระบบต่ำกว่าความจริง ถ้าจะให้ชุดวัดผลสะท้อนระบบจริง ต้องแก้ `eval_retrieval.py` ให้เรียกผ่าน pipeline เดียวกับตอนใช้งาน ผมยังไม่แก้ในรอบนี้เพราะจะทำให้เทียบกับผลรอบ 182 คู่ไม่ได้
...
เรื่องที่ต้องแก้ก่อนอย่างอื่นคือ **golden set** ตราบใดที่คำถามทดสอบยังใช้คำชุดเดียวกับ dataset ตัวเลขจะเอียงเข้าทาง BM25 ตลอด แผนคือเขียน paraphrase ด้วยมือสัก 40 ข้อ ใช้คำคนละชุดกับต้นฉบับ (เช่น ถาม "รถถังรุ่นไหนป้องกันทุ่นระเบิดได้ดี" แทนคำถามต้นฉบับที่มีคำว่า MRAP อยู่) แล้ววัดใหม่ ผลของ `slang` ที่ dense แพ้ขาดเป็นสัญญาณว่าช่องว่างนี้จะยิ่งชัดขึ้น

เรื่องที่สองคือแก้ `eval_retrieval.py` ให้วัดผ่าน pipeline จริงที่มี `normalize_query()` อยู่ด้วย เพราะตอนนี้ชุดวัดผลกับระบบที่ใช้งานจริงไม่ใช่ตัวเดียวกัน
...
hub vector ที่ chunk `[40]`, `[126]`, `[321]` เป็นอาการของโมเดล 384 มิติเหมือนที่เจอใน LAB02 ควรลอง `paraphrase-multilingual-mpnet-base-v2` (768 มิติ) หรือ `intfloat/multilingual-e5-small` ที่ออกแบบมาสำหรับ retrieval โดยเฉพาะ
